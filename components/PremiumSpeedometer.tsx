...
export default function PremiumSpeedometer({
  ...
  // Update display values and status
  useAnimatedReaction(
    () => animatedValue.value,
    (currentValue) => {
      const percent = Math.min(Math.max((currentValue / Math.max(maxValue, 1)) * 100, 0), 100);
      runOnJS(setDisplayValue)(Math.floor(currentValue));
      runOnJS(setDisplayPercentage)(Math.floor(percent));
      ...
      <View style={styles.speedometerWrapper}>
        ...
        <Animated.View style={scaleStyle}>
          <Svg width={size} height={size * 0.7} viewBox={`0 0 ${size} ${size * 0.7}`}>
            <Defs>
              {/* Main gradient for background arc */}
              <LinearGradient id={`bgGradient-${status}`} x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              {/* Needle gradient */}
              <LinearGradient id={`needleGradient-${status}`} x1="0%" y1="0%" x2="0%" y2="100%">
                <Stop offset="0%" stopColor={colors.primary} stopOpacity="1" />
                <Stop offset="100%" stopColor={colors.secondary} stopOpacity="0.8" />
              </LinearGradient>

              {/* Radial gradient for center */}
              <RadialGradient id={`centerGradient-${status}`} cx="50%" cy="50%" r="50%">
                <Stop offset="0%" stopColor={colors.secondary} stopOpacity="1" />
                <Stop offset="100%" stopColor={colors.primary} stopOpacity="0.8" />
              </RadialGradient>

              {/* Outer glow */}
              <RadialGradient id={`glowGradient-${status}`} cx="50%" cy="50%" r="50%">
            ...
            <Path
              ...
              stroke={`url(#needleGradient-${status})`}
            ...
            <Circle
              ...
              cy={centerY}
              r={size * 0.052}
              fill={`url(#centerGradient-${status})`}
            />
      ...
      <View style={styles.detailsGrid}>
        ...
        <View style={styles.detailCard}>
          ...
          <Text
            style={[
              styles.detailValue,
              { color: remaining > 0 ? colors.primary : themeColors.danger },
            ]}
          >
          ...
          >
            <View
              style={[
                styles.detailBarFill,
                {
                  backgroundColor: remaining > 0 ? colors.primary : themeColors.danger,
